...
@
and let us analyize its content
<<>>=
str(samp)
@
As seen from the output, the sampling structure is quite rich and we will show how to specify some of the slots later on.
We  simulate this process by specifying the \code{sampling} argument in the \code{simulate} method
<<>>=
set.seed(123)
X2 <- simulate(mymod, sampling=samp)
@
the sampling structure is recorded along with the data in the \code{yuima} object \code{X2}
<<>>=
str(X2@sampling)
@
...
An alternative tool is the asymptotic expansion method that can often give a solution with accuracy comparable or superior to  
Monte-Carlo methods. The asymptotic expansion method
has an advantage in the computational time because the approximation is given through an analytic formula.
...
\eeas
%
Since $X^{(0)}_t$ is the deterministic solution to the ordinary differential equation
\beas 
...
under the uniform nondegeneracy of the Malliavin covariance of $\tilde{F}^{(\ep)}$.
\footnote{This condition ensures the smoothness of the distribution of $\tilde{F}^{(\ep)}$. 
...
%
In the present situation, each $d_i(g)$ is expressed as 
\beas 
...
just as an example, we shall consider 
...
Given sampled data ${\bf X}_n = (X_{t_i})_{i=0,\ldots,n}$ with $t_i = i\Delta_n$, $\Delta_n \to 0$ as $n\to \infty$,
...
@
Now the \code{yuima} object contains both the \code{model} and the \code{data} slots. We set the initial values for the optimizer as $\theta_1=\theta_2=0.5$ and we specify them as a named list called \code{param.init}. We can now use the function \code{qmle} to estimate the parameters $\theta_1$ and $\theta_2$ as follows
<<echo=TRUE, print=FALSE,results=hide>>=
...
For the consistency of $\hat{\theta}_1$, we are assuming 
...
On the other hand, one needs $T=n\Delta_n\to\infty$ for the consistency of 
$\hat{\theta}_2$ since 
...
the  
consistency of $\hat{\theta}_2$ is obtained. Moreover, asymptotic normality is also established. 
...
the above condition to $n\Delta_n^p\to0$ for $p$ larger than $2$ are extremely important.
...
pretend $\theta_1$ is the unknown parameter for which we construct the Bayesian type estimator $\tilde{\theta}_1$ as  follows
\begin{equation} 
\tilde{\theta}_1
=  
...
under high-frequency samplings, any function $\pi_1$ can be used
if it is positive on $\Theta_1$. 
For the estimation of $\theta_2$, we use $\tilde{\theta}_1$ 
...
and also \citet{Yoshida92} and \citet{UchidaYoshida12} for details. 
...
The process $X_l$ is supposed to be observed  
...
Each $T^{l,j}$ may be a stopping time, so it possibly depends on 
the history of $(X_1,X_2)$ as well as on the precedent stopping times.  
...
\end{equation}
The target variable $\theta$ is random in general and
...
It is known that $U_n$ is consistent and has asymptotically mixed normal 
distribution 
...
the stochastic processes. Under this scheme the data become asynchronous. It is known that 
\begin{equation}
...
an estimate of the quadratic covariation. For example, for the complete data in Figure~\ref{fig:plot-cceplot1}, we obtain the
following estimates
<<>>=
cce(X)
@
...
@
<<plot-cceplot1,echo=FALSE, fig=FALSE, print=FALSE,results=hide>>=
pdf("plot-cceplot1.pdf",width=9,height=5)
par(mar=c(4,4,0,0))
plot(X,main="complete data")
dev.off()
@
\begin{figure}[H]
\includegraphics[width=\textwidth]{plot-cceplot1}
\caption{The complete simulated data.}
\label{fig:plot-cceplot1}
\end{figure} 
...
@
Now we use the \code{subsampling} function to subsample the original data \code{X} into new asynchronous data \code{Y} (see Figure~\ref{fig:plot-cceplot2})
<<>>=
...
@
<<plot-cceplot2,echo=FALSE, fig=FALSE, print=FALSE,results=hide>>=
pdf("plot-cceplot2.pdf",width=9,height=5)
par(mar=c(4,4,0,0))
plot(Y,main="asynchronous data")
dev.off()
@
\begin{figure}[H]
\includegraphics[width=\textwidth]{plot-cceplot2}
\caption{The asynchronous data generated from the simulated ones using Poisson random sub-sampling.}
\label{fig:plot-cceplot2}
\end{figure} 
...
@
We apply the same Poisson random sampling so that the object \code{Y} will contain  asynchronous data (see Figure~\ref{fig:plot-cceplot3})
<<>>=
p1 <- 0.2
p2 <- 0.3
newsamp <- setSampling(
...
@
<<plot-cceplot3,echo=FALSE, fig=FALSE, print=FALSE,results=hide>>=
pdf("plot-cceplot3.pdf",width=9,height=5)
par(mar=c(4,4,0,0))
plot(Y,main="asynchronous data (non linear case)")
dev.off()
@
\begin{figure}[H]
\includegraphics[width=\textwidth]{plot-cceplot3}
\caption{The asynchronous data for the nonlinear system.}
\label{fig:plot-cceplot3}
\end{figure} 
...
When $Y=X$ the model above is a diffusion model.
...
now we generate the second trajectory
 with parameters
$\theta_{1.1}=0.2$ and  $\theta_{2.1}=0.4$. For this trajectory, the initial value is set to the last value of the first trajectory stored in \code{x1} and \code{x2} for the two component of the process (see Figure~\ref{fig:plot-cpoint4})
...
plot(yuima)
@
<<plot-cpoint4,echo=FALSE, fig=FALSE, print=FALSE,results=hide>>=
pdf("plot-cpoint4.pdf",width=9,height=5)
par(mar=c(4,4,0,0))
plot(yuima)
dev.off()
@
\begin{figure}[H]
\includegraphics[width=\textwidth]{plot-cpoint4}
\caption{The 2-dimensional trajectory with change point around $\tau=4$.}
\label{fig:plot-cpoint4}
\end{figure} 
As said, the change-point analysis does not consider the information coming from the drift part of the model and \pkg{yuima} ignores this internally.
...
@
First we show that there is no difference in using the complete model or the model without drift. For simplicity, we assume to know the true values of the parameters for $\theta_{1.k}$ and $\theta_{1.k}$ 
<<cpoint5>>=
...
t.est2$tau
@
As it can be seen, the above estimates of the change point are the same for the complete model \code{yuima} and the model without drift \code{noDriftModel}.
\subsubsection{An example of two stage estimation}
In practical situations, the initial values of the parameters are not known and there is the need to provide a preliminary estimators of them. One possible approach is the two stage change-point estimation approach as explained in  \citet{iacyos09}. The idea is to take a small subset of observations at the very beginning and the end of the time series, estimate a
...
We can now refine the estimate of the change point and the parameters by iterating the above procedure.
<<>>=
...
The Least Absolute Shrinkage and Selection Operator (LASSO) is a useful and well studied approach to the problem of model selection and its major advantage is the simultaneous execution of both parameter estimation and variable selection \citep{Tib96,Knight00,Efron}. 
...
The QMLE $\hat{\theta}$ for this model is the solution of the following problem
$$
...
$$
and $\ddot{\mathbb{H}}_n$ is the matrix of second partial derivatives of $\mathbb{H}$ with respect to the vector $\theta$.
...
\end{equation}
where $\hat \alpha_{j}$ and  $\hat \beta_{k}$ are the unpenalized QMLE's of $\alpha_j$ and $\beta_k$ respectively, $\delta_1, \delta_2>0$ and usually taken unitary.
...
Let us consider the full CKLS model \citep{CKLS92}
$$\de X_t = (\alpha+\beta X_t)\de t + \sigma X_t^\gamma\de W_t$$
and let us try to estimate the parameter on  the U.S. Interest Rates monthly data from 06/1964 to 12/1989 (see Figure~\ref{fig:plot-irates}). We prepare the data, the model and the constraints for optimization
...
@
where \code{lambda10} stores the penaly terms $\lambda_0$ and $\gamma_0$ of \eqref{eq:penalty} of the LASSO method.
<<plot-irates,echo=FALSE, fig=FALSE, print=FALSE,results=hide>>=
pdf("plot-irates.pdf",width=9,height=4)
par(mar=c(4,4,0,0))
plot(rates)
dev.off()
@
\begin{figure}[H]
\includegraphics[width=\textwidth]{plot-irates}
\caption{The U.S. Interest Rates monthly data from 06/1964 to 12/1989.}
\label{fig:plot-irates}
\end{figure} 
Now we apply the \code{lasso} function
<<echo=TRUE,results=hide>>=
...
\code{toLatex(my-yuima-obj)} produces the related \LaTeX{} code which can be copied and pasted in a mathematical paper. For example,
<<>>=
a <- c("-3*x1","-x1-2*x2")
